...
export default function Login() {
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden bg-black flex flex-col md:flex-row items-center justify-center font-sans px-4 select-none">
      ...
      <div className="absolute top-[10%] left-[20%] w-[400px] h-[400px] bg-purple-700/30 rounded-full blur-[120px] z-10 mix-blend-screen animate-pulse pointer-events-none"></div>
      <div className="absolute bottom-[10%] right-[20%] w-[400px] h-[400px] bg-orange-600/30 rounded-full blur-[120px] z-10 mix-blend-screen animate-pulse pointer-events-none delay-1000"></div>

      {/* 4. Login Card (Glassmorphism & Gradient Glow) */}
      <div className="relative z-20 w-full max-w-md">
        {/* Glow wrapper for gradient border */}
        <div className="absolute inset-0 bg-gradient-to-br from-purple-600/30 via-orange-500/20 to-purple-900/30 rounded-[34px] blur-lg opacity-60 transform hover:opacity-100 transition-opacity duration-500"></div>
        
        <div className="relative p-6 sm:p-8 bg-black/40 backdrop-blur-2xl border border-white/10 rounded-[30px] shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex flex-col max-h-[95vh] overflow-y-auto [&::-webkit-scrollbar]:hidden transform transition-all duration-700 hover:-translate-y-2 hover:shadow-[0_25px_60px_rgba(107,33,168,0.3)]">
          ...
          <div className="flex flex-col items-center mb-7 text-center">
            ...
            <div className="w-full flex items-center justify-center mb-3">
              <h1 className="text-xl sm:text-2xl font-black text-white/90 tracking-wide uppercase mt-1 border-b-[3px] border-orange-500/70 pb-2 inline-block">
                {displayText}
              </h1>
            </div>
            <p className="text-white/80 font-semibold text-xs sm:text-sm mt-3 max-w-sm leading-relaxed italic border-l-4 border-orange-500 pl-3 bg-black/20 p-2 rounded-r-lg">
          ...
          <form className="w-full flex justify-center flex-col gap-4" onSubmit={handleAuth}>
            ...
            <div className="flex justify-center w-full mt-2">
              <div 
                className="flex items-center gap-4 bg-[#fafafa] border border-gray-300 rounded-sm p-3 sm:px-4 sm:py-3 cursor-pointer shadow-sm w-full max-w-[300px]"
                onClick={() => {
              ...
              >
                <div className="flex items-center justify-center w-7 h-7 sm:w-8 sm:h-8 rounded-sm border-2 bg-white flex-shrink-0 transition-all duration-300" style={{ borderColor: captchaVerified ? 'transparent' : '#c1c1c1' }}>
                   {captchaLoading ? (
                      <span className="animate-spin h-5 w-5 border-2 border-blue-500 border-t-transparent rounded-full"></span>
                   ) : captchaVerified ? (
                      <Check className="w-6 h-6 sm:w-7 sm:h-7 text-green-600 font-bold" strokeWidth={4} />
                   ) : null}
                </div>
                <span className="text-[#222] font-medium text-sm sm:text-[15px] select-none tracking-wide text-left flex-1" style={{ fontFamily: 'Roboto, sans-serif' }}>
                   I'm not a robot
                </span>
                <div className="flex flex-col items-center justify-center flex-shrink-0 opacity-80">
                  <img src="https://www.gstatic.com/recaptcha/api2/logo_48.png" className="w-6 h-6 sm:w-8 sm:h-8 object-contain mb-0.5" alt="reCAPTCHA" />
                  <span className="text-[8px] sm:text-[10px] text-gray-500 -mt-1 tracking-tight">reCAPTCHA</span>
                  <div className="text-[7px] text-gray-400 mt-0.5 space-x-1">
            ...
            {isLogin && (
              <div className="flex items-center justify-between px-2 pt-1 pb-2">
                ...
                <button 
                  ...
                  className={`w-10 h-5 rounded-full p-0.5 transition-colors duration-300 ease-in-out ${rememberMe ? 'bg-gradient-to-r from-purple-600 to-orange-500' : 'bg-white/10'}`}
                >
